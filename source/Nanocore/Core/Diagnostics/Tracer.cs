...
namespace Nanocore.Core.Diagnostics
{
...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        private Tracer(string name, string description)
        {
            ...
            IsEnabled = true;
            // IsEnabled = string.Equals(Environment.GetEnvironmentVariable("WarpLinkEnableTrace"), "true", StringComparison.OrdinalIgnoreCase);
            if (!IsEnabled)
            {
                return;
            }
        ...
        private void Output(TraceEventType type, string format, params object[] args)
        {
            if (IsEnabled)
            {
                OnWrite(TraceSource.Name, type, new StringBuilder(new string(' ', IndentLevel * Trace.IndentSize)).AppendFormat(format, args).ToString());
            }
        }
        ...
        public void TraceAssert(string format, params object[] args)
        {
            if ((_traceMask & TraceKind.Assert) == TraceKind.None)
            {
                return;
            }
